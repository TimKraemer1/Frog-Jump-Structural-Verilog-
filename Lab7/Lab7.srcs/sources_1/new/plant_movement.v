...
module plant_movement1(
    ...
    assign reset_mux = reset_plant_init_pos ? 10'b0100101100 : mux_out;     
    ...
    FDRE #(.INIT(1'b0) ) ff_4 (.C(clk), .CE((frame & game_play) | reset_plant_init_pos), .D(reset_mux[4]), .Q(reg_out[4]));
    FDRE #(.INIT(1'b1) ) ff_5 (.C(clk), .CE((frame & game_play) | reset_plant_init_pos), .D(reset_mux[5]), .Q(reg_out[5]));
    ...
    FDRE #(.INIT(1'b1) ) ff_8 (.C(clk), .CE((frame & game_play) | reset_plant_init_pos), .D(reset_mux[8]), .Q(reg_out[8]));
    FDRE #(.INIT(1'b0) ) ff_9 (.C(clk), .CE((frame & game_play) | reset_plant_init_pos), .D(reset_mux[9]), .Q(reg_out[9]));
